...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown',
                                                options=[
                                                    ...
                                                    [{'label': i, 'value': i} for i in launch_sites_df
                                                ],
                                                value = 'ALL',
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        g = filtered_df.groupby('class').count() 
        g = g.reset_index(level=0)
        fig = px.pie(g, values='Launch Site', 
        names='class', 
...
def get_payload_scat(entered_site,slider_range):
    filtered_df2 = spacex_df.copy()
    if entered_site == 'ALL':        
        ...
        filtered_df2 = filtered_df2.loc[filtered_df2['Launch Site'] == entered_site]
        low, high = slider_range
        mask = (filtered_df2['Payload Mass (kg)'] > low) & (filtered_df2['Payload Mass (kg)'] < high)
        fig = px.scatter(
            filtered_df2[mask], x="Payload Mass (kg)", y="class", 
